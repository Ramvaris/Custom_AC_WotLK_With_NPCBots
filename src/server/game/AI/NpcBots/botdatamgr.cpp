...
void BotDataMgr::DespawnWandererBot(uint32 entry)
{
    ...
    for (auto it = _botsWanderCreaturesToSpawn.begin(); it != _botsWanderCreaturesToSpawn.end(); ++it)
    {
        if (it->first == entry)
        {
            ...
            auto ctitr = _botsExtraCreatureTemplates.find(entry);
            if (ctitr != _botsExtraCreatureTemplates.end())
            {
            ...
            _botsData.erase(entry);
            _botsExtras.erase(entry);
            _botsAppearanceData.erase(entry);
            _botsExtraCreatureEquipmentTemplates.erase(entry);
            _botsExtraCreatureTemplates.erase(entry);
        ...
        else
        {
            ...
            for (auto const& [bot_class, spare_bots] : _spareBotIdsPerClassMap)
            {
                for (uint32 spareBotId : spare_bots)
                {
                    ...
                    if (bracketEntry && BotDataMgr::GetMinLevelForBotClass(bot_class) > bracketEntry->maxLevel)
                        continue;
