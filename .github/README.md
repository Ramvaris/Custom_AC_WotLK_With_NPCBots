...
- **All-Class Equipment** — Grants all weapon + armor proficiency spells on login. WorldScript startup hook re-applies `AllowableClass=-1` to all weapons/armor and clears class restrictions from item-reward quests on every boot (survives upstream DB updates). Spell-only trainer quests stay class-locked. Core patch auto-swaps STR→AGI on plate armor for Rogue/Hunter/Shaman/Druid — every plate item works, talent % AGI multipliers apply correctly.
...
| **All-Class Equipment** | `AllowableClass` set to `-1` on all weapons and armor. Class restrictions removed from item-reward quests (`quest_template_addon.AllowableClasses=0`). Both DB and in-memory patched on every startup via WorldScript — immune to upstream DB updates overwriting the changes. Initial SQL seed uses `9999_99_99_` prefix. |
