...
- **All-Class Equipment** — Any class can wear any armor type (cloth/leather/mail/plate/shield) and wield any weapon type. Class restrictions on set items removed — Warlock in Death Knight tier, Mage with plate and shield, Druid bear in full plate, anything goes. All proficiency spells granted on login + `AllowableClass` removed from all equipment in the DB. Class restrictions on item-reward quests also removed (spell-only trainer quests stay locked). **Custom AGI plate sets** for agility classes: *Nightstalker's Battlegear* (ilvl 264, T10 equivalent) and *Phantom Dreadnaught Battlegear* (ilvl 200, T7 equivalent) — cloned from warrior plate with STR→AGI swap, sold by Ashari. No DBC file edits — uses `item_dbc` DB table for server-side DBC registration. **Startup-safe:** a WorldScript re-applies all patches on every boot, so upstream DB updates adding new class-restricted items/quests are automatically corrected. Config-gated (`CustomRamvaris.AllClassEquip.Enable`).
...
- **All-Class Equipment** — Grants all weapon + armor proficiency spells on login. WorldScript startup hook re-applies `AllowableClass=-1` to all weapons/armor and clears class restrictions from item-reward quests on every boot (survives upstream DB updates). Spell-only trainer quests stay class-locked. Includes 2 custom AGI plate sets (10 items total) registered via `item_dbc` DB table — no DBC file edits needed. Sold by Ashari (NPC 299902).
...
| **AGI Plate Sets** | 10 custom items (entries 80001–80010): *Nightstalker's Battlegear* (ilvl 264, cloned from Sanctified Ymirjar Lord's) and *Phantom Dreadnaught Battlegear* (ilvl 200, cloned from Heroes' Dreadnaught). STR→AGI stat swap, same plate armor values and visuals. Registered in `item_dbc` for server-side DBC loading — no MPQ/client patches needed. Sold by Ashari. |
