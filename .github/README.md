...
- **All-Class Equipment** — Any class can wear any armor type (cloth/leather/mail/plate/shield) and wield any weapon type. Class restrictions on set items removed — Warlock in Death Knight tier, Mage with plate and shield, Druid bear in full plate, anything goes. All proficiency spells granted on login + `AllowableClass` removed from all equipment in the DB. Class restrictions on item-reward quests also removed (spell-only trainer quests stay locked). Relic-family and ranged caster restrictions are also lifted under AllClassEquip: **wands, librams, idols, totems, and sigils** can be equipped by any class. **Automatic STR→AGI plate swap** for agility classes (Rogue/Hunter/Shaman/Druid): when equipping plate armor, STR stats are applied as AGI instead — works with ANY plate item at every item level, and talent % AGI multipliers apply correctly to the swapped value. Zero custom items, zero commands — pure core-level stat interception. **Startup-safe:** a WorldScript re-applies all patches on every boot, so upstream DB updates adding new class-restricted items/quests are automatically corrected. Config-gated (`CustomRamvaris.AllClassEquip.Enable`).
...
- **All-Class Equipment** — Grants all weapon + armor proficiency spells on login. WorldScript startup hook re-applies `AllowableClass=-1` to all weapons/armor and clears class restrictions from item-reward quests on every boot (survives upstream DB updates). Spell-only trainer quests stay class-locked. Core patches also remove class gating for relic-family/ranged caster gear (wands, librams, idols, totems, sigils) and auto-swap STR→AGI on plate armor for Rogue/Hunter/Shaman/Druid — every plate item works, talent % AGI multipliers apply correctly.
...
| **All-Class Equipment** | `AllowableClass` set to `-1` on all weapons and armor. Class restrictions removed from item-reward quests (`quest_template_addon.AllowableClasses=0`). Relic/ranged slot class locks are lifted under AllClassEquip so any class can equip wands, librams, idols, totems, and sigils. Both DB and in-memory patched on every startup via WorldScript — immune to upstream DB updates overwriting the changes. Initial SQL seed uses `9999_99_99_` prefix. |
