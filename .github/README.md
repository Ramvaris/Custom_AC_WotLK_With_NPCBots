...
- **All-Class Equipment** — Any class can wear any armor type (cloth/leather/mail/plate/shield) and wield any weapon type. Class restrictions on set items removed — Warlock in Death Knight tier, Mage with plate and shield, Druid bear in full plate, anything goes. All proficiency spells granted on login + `AllowableClass` removed from all equipment in the DB. Class-restricted quests are also opened broadly: **all non-pure-skill quests are unlocked**, while pure skill-only trainer quests remain class-locked (so no Mage bear form nonsense). Relic-family and ranged caster restrictions are also lifted under AllClassEquip: **wands, librams, idols, totems, and sigils** can be equipped by any class. **Automatic STR→AGI plate swap** for agility classes (Rogue/Hunter/Shaman/Druid): when equipping plate armor, STR stats are applied as AGI instead — works with ANY plate item at every item level, and talent % AGI multipliers apply correctly to the swapped value. Zero custom items, zero commands — pure core-level stat interception. **Startup-safe:** a WorldScript re-applies all patches on every boot, so upstream DB updates adding new class-restricted items/quests are automatically corrected. Config-gated (`CustomRamvaris.AllClassEquip.Enable`).
...
- **All-Class Equipment** — Grants all weapon + armor proficiency spells on login. Also grants ranged fire abilities (`Shoot/Throw` family) and force-sets relevant weapon skills (including Wands) so bows/guns/crossbows/wands/thrown are actually usable by all classes. WorldScript startup hook re-applies `AllowableClass=-1` to all weapons/armor and opens all class-restricted quests except pure skill-only trainer quests on every boot (survives upstream DB updates). Core patches also remove class gating for relic-family/ranged caster gear (wands, librams, idols, totems, sigils) and auto-swap STR→AGI on plate armor for Rogue/Hunter/Shaman/Druid — every plate item works, talent % AGI multipliers apply correctly.
...
- **Lottery Enchants** — Diablo-style random enchantments on ALL acquired weapons/armor (any quality, grey through legendary). Up to 7 enchants per item (cascading 70/60/50/50/50/40/40% chances), each rolling a random stat from a flat pool of 20 types (13 combat stats + 6 resistances + speed) with flat `1-77` value rolls — all values equally likely, thematic max of 77 (7 slots × lucky 7s). Stats are level-scaled: `max(1, round(value × level / 80))`. 7 color tiers (Grey→Red) each spanning 11 values for instant quality recognition. Vendor purchases only roll on Green+ items (grey/white vendor trash skipped to prevent farming). Non-vendor sources (loot, craft, quest, mail) roll on ANY quality. Includes **Movespeed** (1-77% per roll, stacks to +100% = 200% base, affects run+swim+flight, NOT level-scaled — always full value, **skipped when mounted** — mounts always use vanilla 100% base) and **Mobility Boost** (1% chance, 3 stages: 100%/200%/300%, combined with speed bonus up to 600% — full superman mode). Flying mount behavior: jump, press space mid-air to fly (no level restriction). `.flylock` toggles flight on/off. Speed bonus is multiplicative with aura buffs. Gossip-based `.reroll` with Take/Keep preview (500g). Gossip-based `.enchants` viewer with item drill-down and **[Sum] equipped totals page** (aggregated stat overview across all equipped items — instant check for overcapped speed or stat imbalance). Backward-compatible: existing items with old DBC-based enchant IDs still work via fallback decoding. Migration script `tools/migrate_lottery_enchants.py` re-rolls legacy DB entries into the new format. BG flag auto-drops when airborne.
- **Smart Wandering Bots** — Zone-aware dynamic bot spawning instead of global. Spawns MinAmount–MaxAmount bots ONLY in the player's current zone, despawns on zone change. Optional faction balancing to even out Alliance/Horde ratio. **Fixes:**
  - Core bracket distribution edge case: with low spawn counts (1-2 bots), `CalculatePct(1, 15) = 0` caused ALL brackets to get zero bots assigned, so nothing ever spawned. Fixed by scanning the zone's WanderNode level ranges and assigning bots to matching brackets directly.
  - Replaced fatal `ASSERT(!level_nodes.empty())` in wanderer spawn with graceful fallback — prevents server crash when a zone's level range doesn't cover the requested bracket (e.g. bracket 7 in Elwynn Forest).
  - Balanced-faction mode now guarantees spawns even with low counts by biasing odd remainders to the player's faction and falling back across factions when one has no spare bots/nodes.
...
| **All-Class Equipment** | `AllowableClass` set to `-1` on all weapons and armor. Class-restricted quests are unlocked except pure skill-only trainer quests (`quest_template_addon.AllowableClasses=0` for non-skill-only). Relic/ranged slot class locks are lifted under AllClassEquip so any class can equip wands, librams, idols, totems, and sigils. Both DB and in-memory patched on every startup via WorldScript — immune to upstream DB updates overwriting the changes. Initial SQL seed uses `9999_99_99_` prefix. |
| **Plate STR→AGI Swap** | Core patch in `Player::_ApplyItemBonuses`: when a Rogue, Hunter, Shaman, or Druid equips plate armor (`ITEM_SUBCLASS_ARMOR_PLATE`), `ITEM_MOD_STRENGTH` is applied as `ITEM_MOD_AGILITY` instead. Works at stat application time so talent % AGI multipliers (e.g. Sinister Calling, Lightning Reflexes) apply correctly to the converted value. No custom items, no commands — fully automatic. |
