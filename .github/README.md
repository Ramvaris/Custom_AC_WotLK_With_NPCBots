...
- **All-Class Equipment** — Grants all weapon + armor proficiency spells on login. WorldScript startup hook re-applies `AllowableClass=-1` to all weapons/armor and clears class restrictions from item-reward quests on every boot (survives upstream DB updates). Spell-only trainer quests stay class-locked. Vendors sell all class sets to everyone. For FUN.
