...
### Fixes (vs upstream)
- **Withdraw Lag Fix**: Switched from async `Execute()` to synchronous `DirectExecute()` for withdraw operations. Fixes the "1-stack-behind" display bug where the gossip menu showed stale amounts after withdrawing. Uses raw SQL strings (not prepared statements), so `DirectExecute` works safely on the synchronous connection. Safe for multi-player concurrent access because each player's withdraw operates on their own `character_id` rows — no cross-player data races.
